...
// If credentials are missing, provide fallback behavior
if (!PAYPAL_CLIENT_ID || !PAYPAL_CLIENT_SECRET) {
  console.log("PayPal credentials not configured - using mock responses");
}
  ...
  try {
    if (!ordersController) {
      return res.status(200).json({ 
        id: "mock_order_id", 
        status: "CREATED",
        message: "PayPal not configured - using mock response" 
      });
    }
